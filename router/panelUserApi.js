    ...
    try{const data={
        ...
        const reportList = await customer.aggregate([
            ...
            { $match:data.customer?{$or:[
                ...
                {customerID:new RegExp('.*' + data.customer + '.*')},
                {mobile:new RegExp('.*' + data.customer + '.*')}
            ]}:{}},
            { $match:data.phone?{$or:[
                {phone:new RegExp('.*' + data.phone + '.*')},
                {mobile:new RegExp('.*' + data.phone + '.*')}
            ]}:{}}
        ])
...
router.post('/update-customer',jsonParser,async (req,res)=>{
    ...
    const data = {
      ...
      userNameBI: req.body.userNameBI,
      passwordBI: req.body.passwordBI
    };
    ...
    try{
        ...
        {$set:data})
       res.json({data:userData,query:data,
       success:"تغییرات اعمال شدند"})
    }
    catch(error){
        res.status(500).json({message: error.message})
    } 
